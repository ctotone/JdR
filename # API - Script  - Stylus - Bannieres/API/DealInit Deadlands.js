...
/* property
    ...
    indexOf, length, jokername, longName, makeDeck, name, parse, playerid, pr, push,
...
var DealInit = DealInit || (function() {
  ...
  var version = '1.9',
    ...
    stackMakeDeck = function() {
      ...
      this.cards[52] = new Card(52, "J.Noir", "Black Joker");
      this.cards[53] = new Card(53, "J.Rouge", "Red Joker");
      if (fourJokers > 0) {
        this.cards[54] = new Card(54, "J.Argent", "Silver Joker");
        this.cards[55] = new Card(55, "J.Jaune", "Yellow Joker");
      }
    ...
    display = function(id) {
      ...
      if (!deck.cards) {
        sendChat('', '/w ' + who + '  Paquet non initialisé ! Tapez : !deal-init --reset');
        return;
      ...
      sendChat('', '/w ' + who + ' ' + divStart + '<div style="font-weight: bold; border-bottom: 1px solid black;font-size: 130%;">' +
        'DealInit : cartes dans la pioche</div>' + s + divEnd);
      ...
      sendChat('', '/w ' + who + ' ' + divStart + '<div style="font-weight: bold; border-bottom: 1px solid black;font-size: 130%;">' +
        'DealInit : cartes en initiative</div>' + s + divEnd);
      ...
      sendChat('', '/w ' + who + ' ' + divStart + '<div style="font-weight: bold; border-bottom: 1px solid black;font-size: 130%;">' +
        'DealInit : défausse</div>' + s + divEnd);
    },
    ...
    getInitiativeEdges = function(id) {
      ...
      if (!turnorder.length) {
        sendChat('', '/w ' + who + ' ' + divStart + '<div style="font-weight: bold; border-bottom: 1px solid black;font-size: 130%;">' +
          'Ordre d’initiative vide ! </div>' + divEnd);
        return;
      }
      ...
      for (i = 0; i < turnorder.length; i += 1) {
        ...
        if (turnorder[i].pr === "H" || turnorder[i].pr === "h") {
          ...
          sendChat('', '/w gm ' + char_name + ' est en attente ce tour-ci !');
        }
        ...
        if (toid === "-1") {
          ...
          initEdges[i] = {
            ...
            jokername: turn_order_short_name,
            layer: "none"
          };
        ...
        // turn order item is a "token that represents a character", look for init edges
          ...
          character_name_court = getAttrByName(char_obj.id, "character_name_court") || "";
          ...
          initEdges[i] = {
            ...
            toktype: tokentype,
            shortname: turn_order_short_name,
            jokername: character_name_court || turn_order_short_name,
            layer: onlayer
          };
    ...
    doTheDeal = function(id) {
      ...
      if (onlyToString.length > 0) {
        onlyToActive = 1;
        sendChat('', '/w gm Mode onlyto actif. Recherche du nom contenant : ' + '<u>' + onlyToString + '</u>');
      }
      ...
      if (deck.cardCount() === 0) {
        sendChat('', '/em Plus de cartes dans la pioche — mélange effectué.');
        deck.combine(discards);
        shuffle();
      }
      ...
      if (jokerLastRound === 1 && !onlyToActive && !dealToChat) {
        sendChat('', '/em ' + divStart + '<div style="font-weight: bold; border-bottom: 1px solid black;font-size: 130%;">' +
          'Joker au tour précédent !' + '</div>Mélange du paquet...' + divEnd);
        discards.combine(hand);
        deck.combine(discards);
        shuffle();
        jokerLastRound = 0;
        ...
        if ((!onlyToActive) || (onlyToActive && initEdges[i].name.indexOf(onlyToString) !== -1) || dealToChat) {
          ...
          // deal to chat  xxx
            ...
            if (dealToChatActive) {
              ...
              sendChat('', '/em Carte tirée :  ' + nextcard.shortName);
            ...
            if (initEdges[i].edges.indexOf('TT') !== -1) {
              ...
              nextcard = deck.deal();
              // send the card drawn to chat
              sendChat('', sendto + divStart + '<div style="font-weight: bold; border-bottom: 3px solid green;font-size: 100%;">' + '<u>' + initEdges[i].shortname + '...</u> Carte de Tacticien !' + '</div>' + nextcard.shortName + divEnd);
            ...
            if (initEdges[i].edges.indexOf('MTT') !== -1) {
              ...
              nextcard = deck.deal();
              // send the card drawn to chat
              sendChat('', sendto + divStart + '<div style="font-weight: bold; border-bottom: 3px solid green;font-size: 100%;">' + '<u>' + initEdges[i].shortname + '...</u> Carte de Maître tacticien !' + '</div>' + nextcard.shortName + divEnd);
          ...
          if ((turnorder[i].rank > 51 && turnorder[i].rank < 60) || jokerInChat) {
            ...
            // Message affiché publiquement lorsqu'un Joker devient la carte d'initiative du personnage.
            if (turnorder[i].rank > 51 && turnorder[i].rank < 60) {
              sendChat('', '/em ' + divStart + '<div style="font-weight: bold; border-bottom: 1px solid black;font-size: 130%;">' +
                initEdges[i].jokername + ' tire un Joker !</div>' +
                'Un Joker vient d\'être tiré ! Le personnage peut agir quand bon lui semble durant le round, avec la possibilité d’interrompre l’action de quelqu’un d’autre.<br>' +
                'Tous tes jets de Traits et de dégâts se font avec un bonus de 2 !' + divEnd);
            }
            ...
            if (initEdges[i].edges.indexOf('WCE') !== -1) {
              ...
              sendChat('', sendto + divStart + '<div style="font-weight: bold; border-bottom: 1px solid black;font-size: 130%;">' +
                initEdges[i].shortname + '</div>Votre Joker active votre <b>Atout de Joker !</b>' + divEnd);
            }
            jokerInChat = 0;
            if (verboseMode) {
              var whoVM = getObj('player', id).get('_displayname').split(' ')[0];
          ...
          if (turnorder[i].rank !== "-1" && turnorder[i].rank !== "60" && turnorder[i].pr.indexOf("clubs") !== -1 && showComps ) {
            sendChat('', sendCompTo + 'Complication pour ['+initEdges[i].shortname+'] si c’est une tâche dramatique ou une poursuite !');
          }
    ...
    showHelp = function(id) {
      ...
      sendChat('',
        ...
        '<p>DealInit gère une initiative à base de cartes, dans le style Savage Worlds, en distribuant des cartes dans l’ordre d’initiative et en triant automatiquement l’ordre de jeu.</p>' +
        '<p>Le script n’utilise pas le système de paquet de cartes de Roll20. Il gère son propre paquet interne, qui est remélangé quand la pioche est vide ou lorsqu’un Joker a été tiré.</p>' +
        '<p>Il vérifie aussi les attributs des personnages afin de gérer les Atouts et Handicaps d’initiative Savage Worlds.</p>' +
        '<p>Les Atouts d’initiative doivent être indiqués dans un attribut nommé InitEdges, sous forme de liste séparée par des virgules. Exemple : Qui,LH</p>' +
        '<p>Les codes reconnus sont les suivants :</p>' +
        '<p><b>Qui</b> = Vif</p>' +
        '<p><b>LH</b> = Sang-froid</p>' +
        '<p><b>ILH</b> = Sang-froid amélioré</p>' +
        '<p><b>TT</b> = Tacticien (carte tirée dans le chat)</p>' +
        '<p><b>MTT</b> = Maître tacticien (carte tirée dans le chat)</p>' +
        '<p><b>HH</b> = Hésitant</p>' +
        '<p><b>WCE</b> = Atout activé par Joker</p>' +
        '</div>' +
        '<b>Commands</b>' +
        ...
        '<b><span style="font-family: serif;">' + '(<i>no args</i>)' + '</span></b> ' + ' Distribue les cartes dans l’ordre d’initiative et trie automatiquement.' +
        '</li> ' +
        '<li style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;">' +
        '<b><span style="font-family: serif;">' + '--help' + '</span></b> ' + ' Affiche cette aide.' +
        '</li> ' +
        '<li style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;">' +
        '<b><span style="font-family: serif;">' + '--reset' + '</span></b> ' +
        ' Réinitialise le paquet avec 2 Jokers et le mélange. À utiliser au début d’une nouvelle scène ou rencontre. Ajoutez '+
        '<b><span style="font-family: serif;">' + '--quiet' + '</span></b> pour réduire les messages dans le chat.' +
        '</li> ' +
        '<li style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;">' +
        '<b><span style="font-family: serif;">' + '--4jreset' + '</span></b> ' +
        ' Réinitialise le paquet avec 4 Jokers et le mélange. À utiliser au début d’une nouvelle scène ou rencontre. Ajoutez '+
        '<b><span style="font-family: serif;">' + '--quiet' + '</span></b> pour réduire les messages dans le chat.' +
        '</li> ' +
        '<li style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;">' +
        '<b><span style="font-family: serif;">' + '--clearTurnOrder' + '</span></b> ' + ' Vide entièrement l’ordre d’initiative. Utile après un changement de page.' +
        '</li> ' +
        '<li style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;">' +
        '<b><span style="font-family: serif;">' + '--show' + '</span></b> ' + ' Affiche le contenu actuel de la pioche, de la défausse et de l’ordre d’initiative.' +
        '</li> ' +
        '<li style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;">' +
        '<b><span style="font-family: serif;">' + '--onlyto --string' + '</span></b> ' + ' distribue une carte uniquement aux noms contenant "string". Attention : sensible à la casse.' +
        '</li> ' +
        '<li style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;">' +
        '<b><span style="font-family: serif;">' + '--deal2chat' + '</span></b> ' + ' tire une carte dans le chat, par exemple lorsqu’un joueur dépense un jeton pour retirer.' +
        '</li> ' +
        '<li style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;">' +
        '<b><span style="font-family: serif;">' + '--showComps' + '</span></b> ' + ' signale les complications dans le chat lorsqu’une carte de trèfle est tirée.' +
        '</li> ' +
        '</ul>' +
        '</div>' +
        '</div>' +
